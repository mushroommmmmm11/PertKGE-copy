...
# ===== Few-shot relation dropout wrapper =====
class ComplExModelWithFewshotDropout(ComplExModel):
    """ComplExModel with dropout on few-shot relation embeddings during training."""

    def __init__(self, emb_dim, n_entities, n_relations,
                 fewshot_rel_ids=None, fewshot_dropout=0.5):
        super().__init__(emb_dim, n_entities, n_relations)
        self.fewshot_rel_ids = fewshot_rel_ids or []
        self.fewshot_dropout = fewshot_dropout
    ...
    def scoring_function(self, h_idx, t_idx, r_idx):
        ...
        # Apply dropout to few-shot relation embeddings (shared mask for real+imag)
        if self.training and self.fewshot_rel_ids:
            mask = torch.ones(r_idx.shape[0], self.emb_dim, device=re_r.device)
            for rid in self.fewshot_rel_ids:
                hit = (r_idx == rid)
                if hit.any():
                    drop_mask = torch.bernoulli(
                        torch.full((hit.sum(), self.emb_dim),
                                   1 - self.fewshot_dropout, device=re_r.device)
                    )
                    # Inverse scaling to preserve expected activation magnitude
                    drop_mask = drop_mask / (1 - self.fewshot_dropout)
                    mask[hit] = drop_mask
            re_r = re_r * mask
            im_r = im_r * mask
