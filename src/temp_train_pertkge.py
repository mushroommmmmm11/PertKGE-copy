...
def five_fold_cv(args):
    ...
    for i in range(5):
        ...
        existing_ckpts = [f for f in os.listdir(args.save_model_path) if re.match(f"pertkg{i}_epoch\\d+\\.pt", f)]
        if existing_ckpts:
            latest_ckpt = sorted(existing_ckpts, key=lambda x: int(re.search(r'epoch(\\d+)', x).group(1)))[-1]
            resume_checkpoint = os.path.join(args.save_model_path, latest_ckpt)
        ...
        for epoch in range (start_epoch, args.nepoch):
            running_loss = 0.0
            model.train()
            print(f"model 所在设备：{next(model.parameters()).device}")
            ...
            if epoch > args.warm_up:
                if MRR > best_mrr:
                    ...
                    patients = 0
                    if args.save_model:
                        best_model_path = os.path.join(args.save_model_path, f"best_model_fold_{i}.pt")
                        torch.save({
                            'epoch': best_epoch,                                                                                   'model_state_dict': model.state_dict(),                                                                'optimizer_state_dict': optimizer.state_dict(),
                            'best_mrr': best_mrr
                                                                                                                               }, best_model_path)
                        print(f" New best model saved for fold {i} at epoch {best_epoch} (MRR={best_mrr:.5f})")
                else:
                    patients += 1

                if patients >= args.patients:
                    print(f" Early stopping at epoch {epoch+1} for fold {i}, best MRR={best_mrr:.5f}")
                    break
        ...
        if args.mode == 'reproduce':
            ...
            checkpoint = torch.load(os.path.join(args.save_model_path, f"best_model_fold_{i}.pt"))
            model.load_state_dict(checkpoint['model_state_dict'])
            print(f" Loaded best model from fold {i} (epoch {checkpoint['epoch']}, MRR={checkpoint['best_mrr']:.5f})")
